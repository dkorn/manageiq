class ContainerDeployment < ApplicationRecord
  ...
  def addresses_array(container_deployment_nodes)
    ...
    container_deployment_nodes.each do |container_deployment_node|
      ip = container_deployment_node.node_address
      ip_array << ip if ip
    end
    ip_array
  end
...
EOS
  ...
  def create_deployment(params, user)
    ...
    if method_type.include? "existing_managed"
      ...
      params["ssh_authentication"]["userid"] = deployed_on_ems.kind_of? ManageIQ::Providers::Amazon::CloudManager ? "ec2-user" : "root"
    end
  ...
  def generate_roles
    ...
    unless identity_provider_auth.first.htpassd_users.empty?
      ...
      result["master"]["openshift_master_htpasswd_users"] = htpass_hash.to_json
    end
